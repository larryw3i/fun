...
class HomestickerListView(ListView):
    ...
    def get_queryset(self):
        queryset = Homesticker.objects.filter(is_hidden=False).order_by(
            "-promulgating_date"
        )
        return queryset
...
class HomestickerDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        file_path = os.path.join(
            settings.MEDIA_ROOT, str(context_data["object"].content_file.name)
        )
        context_data["is_video"] = str(
...
class FunhomestickerListView(ListView):
    ...
    def get_queryset(self):
        queryset = Funhomesticker.objects.filter(is_hidden=False).order_by(
            "-promulgating_date"
        )
        return queryset
...
def get_all_bootswatch_themes(request):
    if request.method == "GET":
        return JsonResponse(
            os.listdir(
                os.path.join(
                    settings.BASE_DIR, "static", "libs", "bootswatch", "dist"
                )
            ),
            safe=False,
        )
...
def get_default_homesticker():
    ...
    default_carousel_content_file_path = os.path.join(
        ...
        "media",
        "default_carousel_content_file.pdf",
    )
    return Homesticker(
        title=_("X-DOVE"),
        subtitle=_("Larry`s sharing"),
        cover=os.path.exists(default_carousel_image_path)
        and File(open(default_carousel_image_path))
        or None,
        promulgator=User.objects.get(is_superuser=True),
        content_file=os.path.exists(default_carousel_content_file_path)
        and File(open(default_carousel_content_file_path))
        or None,
...
def data_privacy(request):
    if request.method == "GET":
        return render(
            request,
            os.path.exists(data_privacy_template_path)
            and data_privacy_template
            or data_privacy_example_template,
        )
...
def legal_information(request):
    if request.method == "GET":
        return render(
            ...
            and legal_information_template
            or legal_information_example_template,
        )
