...
class Homesticker(models.Model):
    class Meta:
        verbose_name = _("Home sticker(deprecation)")
        verbose_name_plural = _("Home sticker(deprecation)")
...
class Appreciation(models.Model):
    ...
    invitee = models.CharField(max_length=8, verbose_name=_("invitee"))

    invitee_title = models.CharField(
        max_length=8, verbose_name=_("invitee title")
    )

    brief_comment = models.CharField(
        max_length=16, verbose_name=_("brief comment")
    )

    illustration = models.ImageField(
        upload_to=upload_to, verbose_name=_("illustration")
    )
    ...
    content = fields.RichTextUploadingField(
        max_length=4096, verbose_name=_("appreciation content")
    )

    submitter = models.ForeignKey(
        to=Funuser, on_delete=models.CASCADE, verbose_name=_("Submitter")
    )

    submitting_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Submitting date")
    )
