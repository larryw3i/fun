...
def get_classification_issue_url():
    return (
        "https://github.com/larryw3i/fun/blob/master/fun/templates/"
...
def get_top_filter_path(context):
    ...
    top_filter_html = (
        "eduhub/_top_filters/_eduhub_base_top_filter."
...
def curr_classification(context):
    ...
    if not re.match("[\\w]{8}(-[\\w]{4}){3}-[\\w]{12}", _id):
        _id = uuid.UUID(int=0)
    classification = Classification.objects.filter(id=_id).first()
...
def user_in_asgroup(context):
    request = context["request"]
    return (
        ASharingGroupMember.objects.filter(
            funuser=request.user, enable=True
        ).exists()
        and "1"
        or ""
    )
