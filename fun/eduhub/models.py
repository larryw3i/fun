...
class Classification(models.Model):
    ...
    def __str__(self):
        return ("" if self.parent is None else str(self.parent) + "/") + _(
            self.name
        )
    ...
    comment = models.CharField(
        ...
        blank=True,
        max_length=64,
        verbose_name=_("Classification comment"),
    )
...
class Label(models.Model):
    ...
    name = models.CharField(
        max_length=64, blank=False, verbose_name=_("Label name")
    )
...
class Funcontent(models.Model):
    ...
    title = models.CharField(
        max_length=64, blank=False, verbose_name=_("Content title")
    )

    content = RichTextUploadingField(
        max_length=2048, verbose_name=_("Content")
    )
...
class Eduhubhomesticker(models.Model):
    ...
    title = models.CharField(
        max_length=32, verbose_name=_("Eduhub homepage sticker title")
    )

    subtitle = models.TextField(
        max_length=64, verbose_name=_("Eduhub homepage sticker subtitle")
    )

    cover = models.ImageField(
        upload_to=upload_to, verbose_name=_("Eduhub homepage sicker cover")
    )
    ...
    comment = models.TextField(
        max_length=128, verbose_name=_("Eduhub homepage sticker comment")
    )
...
class Funtest(models.Model):
    ...
    test_title = models.CharField(
        max_length=64, blank=True, null=True, verbose_name=_("Test commit")
    )

    test_owner = models.ForeignKey(
        to=Funuser, on_delete=models.CASCADE, verbose_name=_("Test owner")
    )

    test_text = models.TextField(
        max_length=12288, verbose_name=_("Template text")
    )

    submitting_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Test submitting date")
    )

    last_modifying_date = models.DateTimeField(
        blank=True, null=True, verbose_name=_("Last modifying date")
    )
...
class Appraising(models.Model):
    ...
    comment = models.CharField(max_length=64, verbose_name=_("Comment"))
...
class ASharingContent(models.Model):
    ...
    DOU = models.DateTimeField(
        auto_now=True, verbose_name=_("Date of appraising content updating")
    )
...
class ASharingGroup(models.Model):
    ...
    is_legal = models.BooleanField(default=True, verbose_name=_("Is legal?"))
...
class ASGMemberClassification(models.Model):
    ...
    cname = models.CharField(
        max_length=64, blank=False, verbose_name=_("Classification Name")
    )
    comment = models.CharField(
        null=True,
        blank=True,
...
class ASharingGroupMember(models.Model):
    ...
    mname = models.CharField(
        max_length=64, blank=False, verbose_name=_("Member Name")
    )
    funuser = models.ForeignKey(
        null=False,
        to=Funuser,
        on_delete=models.CASCADE,
        verbose_name=_("funuser"),
    )
    agroup = models.ForeignKey(
        to="ASharingGroup",
        on_delete=models.CASCADE,
    ...
    gclassification = models.ManyToManyField(
        to="ASGMemberClassification",
        blank=True,
        verbose_name=_("classifications"),
    )
    isjudge = models.BooleanField(default=False, verbose_name=_("Is judge ?"))
