...
class LabelCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if not form.instance.cover.file:
            ...
            return render(
                self.request, label_create_template, context={"form": form}
            )
...
class LabelListView(ListView):
    ...
    def get_queryset(self):
        ...
        ) or self.request.COOKIES.get("is_my_label_list", False):
            return Label.objects.filter(is_legal=True).order_by(
                "-creating_date"
            )

        else:
            return Label.objects.filter(
                is_legal=True, author=self.request.user
...
class LabelDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        if not Label.objects.filter(pk=self.pk, author=request.user).exists():
            raise Http404()
...
class FuncontentListView(ListView):
    ...
    def get_queryset(self):
        return (
            Funcontent.objects.filter(
                label__id=self.kwargs["label_id"], is_legal=True
    ...
    def get_context_data(self, **kwargs):
        ...
        if len(str(self.kwargs.get("label_id", ""))) > 0:
            ...
            context_data["is_author"] = (
                Label.objects.get(id=self.kwargs["label_id"]).author
                == self.request.user
            )
        return context_data
...
class FuncontentCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        ...
        initial["classification"] = (
            Classification.objects.get(pk=classification_id)
            if Classification.objects.filter(pk=classification_id).exists()
...
class FuncontentDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not Funcontent.objects.filter(
            pk=self.pk, label__author=request.user
        ).exists():
            raise Http404()
    ...
    def get_success_url(self):
        return reverse(
            "eduhub:funcontent_list", kwargs={"label_id": self.label_id}
        )
...
class FuncontentUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "eduhub:funcontent_list", kwargs={"label_id": self.object.label.id}
        )
            ...
            funcontents = (
                Funcontent.objects.filter(
                    (
                        ...
                        | Q(
                            classification__icontains=self.request.GET.get("q")
                        )
                    )
                    & Q(classification__icontains=eduhub_top_filter)
                ).order_by("-uploading_date")
                if len(eduhub_top_filter) > 0
                else Funcontent.objects.filter(
                    Q(title__icontains=self.request.GET.get("q"))
...
class AppraisingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        asharingcontent = ASharingContent.objects.get(id=self.kwargs["ac_id"])
        context["asharingcontent"] = asharingcontent
        return context

    def form_valid(self, form):
        if not ASharingGroupMember.objects.filter(
            funuser=self.request.user, enable=True, isjudge=True
        ).exists():
            return Http404()
        form.instance.acontent = ASharingContent.objects.get(
            id=self.kwargs["ac_id"]
        )
        form.instance.amember = ASharingGroupMember.objects.get(
            funuser=self.request.user
        )
        return super().form_valid(form)

    def get_success_url(self):
        asg_id = ASharingContent.objects.get(id=self.kwargs["ac_id"]).agroup.id
        return reverse("eduhub:appraising_c_list", kwargs={"asg_id": asg_id})
...
class ASharingCListView(ListView):
    ...
    def get_queryset(self):
        return ASharingContent.objects.filter(
            is_legal=True, agroup__id=self.kwargs["asg_id"]
        ).order_by("-DOU")
...
class ASharingCCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        initial = super().get_initial()
        classification_id = self.request.COOKIES.get("classification", None)
        initial["classification"] = (
            Classification.objects.get(pk=classification_id)
    ...
    def form_valid(self, form):
        ...
        form.instance.agroup = ASharingGroup.objects.get(
            id=self.kwargs["asg_id"]
        )
        return super().form_valid(form)
...
class ASharingCUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        if not ASharingContent.objects.filter(
...
class ASharingCDetailView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(**kwargs)
        context["user_is_judge"] = ASharingGroupMember.objects.filter(
            funuser=self.request.user, enable=True, isjudge=True
        ).exists()
...
class ASGMemberListView(ListView):
    ...
    def get_queryset(self):
        return ASharingGroupMember.objects.filter(is_legal=True).order_by(
            "-DOU"
        )
...
class ASGroupListView(ListView):
    ...
    def get_queryset(self):
        gmembers = ASharingGroupMember.objects.filter(
            enable=True, funuser=self.request.user
        ).select_related("agroup")
